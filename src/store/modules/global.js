...
const sessionCookieName = 'prodo_app';
...
const getters = {
	...
	isSuperAdmin: (state) => {
		let roles = state.user && state.user.roles ? state.user.roles : '[]';
		if (typeof (roles) === 'string') roles = JSON.stringify(roles);
		const isValid = roles.includes('super_admin');
		return isValid;
	},
	isClient: (state) => {
		let roles = state.user && state.user.roles ? state.user.roles : '[]';
		if (typeof (roles) === 'string') roles = JSON.stringify(roles);
		const isValid = roles.includes('client');
		return isValid;
	},
