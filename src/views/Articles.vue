	...
	<div class="page page-article">
		<div class="container">
			<div class="d-flex justify-content-between">
				<div class="mb-4">
					<h2>{{ $t('Articles') }}</h2>
					<div>{{ $t('Manage all articles data') }}</div>
				</div>
			</div>
			<!-- Table Menu -->
				...
				<div class="col-md-6 mb-2">
					<div class="d-flex justify-content-start">
						...
						<md-menu md-align-trigger :md-offset-x="0">
							...
							<md-menu-content>
								<md-menu-item @click="isImportArticleVisible = true"><span class="minw-87">{{ $t('Import Article') }}</span></md-menu-item>
								<md-menu-item @click="isExportArticleVisible = true"><span class="minw-87">{{ $t('Export Article') }}</span></md-menu-item>
							</md-menu-content>
						</md-menu>
					</div>
				</div>
			...
			<div class="card">
				<div class="table-responsive">
					<table class="table">
						<thead>
							<tr>
								<th width="3%"><input type="checkbox" v-model="selectAll" @click="select"><i class="form-icon"></i></th>
								<th width="5%">{{ $t('No') }}</th>
								<th>{{ $t('Title') }}</th>
								<th>{{ $t('Category') }}</th>
		...
		<div class="modals">
			...
			<modal
				...
				<div>
					<a href="./../../template-xlsx/Template_Articles.xlsx" download>{{ $t('Download') }} Excel Template - {{ $t('Articles') }}</a>
				</div>
				</template>
				<template slot="footer">
					<div class="w-100 flex-center">
						<button @click="acceptImport" :disabled="!tmpArticles" type="submit" class="btn btn-primary" :class="{ 'is-loading': isImporting }">
							<span>{{ $t('Upload') }}</span>
						</button>
						<button @click="isImportArticleVisible = false" type="submit" class="btn btn-default">
							<span>{{ $t('Cancel') }}</span>
						</button>
					</div>
				</template>
